...
def diffx(kx,ky,M):
    delta=1e-15
    diffx=(H(kx+delta,ky,M)-H(kx,ky,M))/delta
    return diffx
def diffy(kx,ky,M):
    delta=1e-15
    diffy=(H(kx,ky+delta,M)-H(kx,ky,M))/delta
    return diffy
# print(diffx(0,0))
...
for i in range(len(kx)-1):
     for j in range(len(ky)-1):
        ...
        partial01x=np.dot(sta0.transpose().conj(),np.dot(diffx(kx[i],ky[j],M),sta1))*np.dot(sta1.transpose().conj(),np.dot(diffy(kx[i],ky[j],M),sta0))
        partial01y=np.dot(sta0.transpose().conj(),np.dot(diffy(kx[i],ky[j],M),sta1))*np.dot(sta1.transpose().conj(),np.dot(diffx(kx[i],ky[j],M),sta0))
        chern_1=chern_1+((partial01x-partial01y)/((eng0-eng1)**2))*Delta**2

for i in range(len(kx) - 1):
    for j in range(len(ky) - 1):
        eng0, eng1, sta0, sta1 = calate(kx[i], ky[j],M)
        partial10x = np.dot(sta1.transpose().conj(), np.dot(diffx(kx[i], ky[j],M), sta0)) * np.dot(sta0.transpose().conj(), np.dot(diffy(kx[i], ky[j],M), sta1))
        partial10y = np.dot(sta1.transpose().conj(), np.dot(diffy(kx[i], ky[j],M), sta0)) * np.dot(sta0.transpose().conj(), np.dot(diffx(kx[i], ky[j],M), sta1))
...
for k in range(len(Mchange)):
    ...
    for i in range(len(kx) - 1):
        for j in range(len(ky) - 1):
            eng0, eng1, sta0, sta1 = calate(kx[i], ky[j], Mchange[k])
            partial01x = np.dot(sta0.transpose().conj(), np.dot(diffx(kx[i], ky[j],Mchange[k]), sta1)) * np.dot(
                sta1.transpose().conj(), np.dot(diffy(kx[i], ky[j],Mchange[k]), sta0))
            partial01y = np.dot(sta0.transpose().conj(), np.dot(diffy(kx[i], ky[j],Mchange[k]), sta1)) * np.dot(
                sta1.transpose().conj(), np.dot(diffx(kx[i], ky[j],Mchange[k]), sta0))
            chern_change1 = chern_change1 + ((partial01x - partial01y) / ((eng0 - eng1) ** 2)) * Delta ** 2
    for i in range(len(kx) - 1):
        for j in range(len(ky) - 1):
            eng0, eng1, sta0, sta1 = calate(kx[i], ky[j],Mchange[k])
            partial10x = np.dot(sta1.transpose().conj(), np.dot(diffx(kx[i], ky[j],Mchange[k]), sta0)) * np.dot(
                sta0.transpose().conj(), np.dot(diffy(kx[i], ky[j],Mchange[k]), sta1))
            partial10y = np.dot(sta1.transpose().conj(), np.dot(diffy(kx[i], ky[j],Mchange[k]), sta0)) * np.dot(
